        ...
        try:
            for text in probes:
                ...
                for segment in self._split(text):
                    spliced.extend(
                        self._tokenizer(segment, add_special_tokens=False)["input_ids"]
                    )
            ...
            logger.warning(
                "Incremental tokenizer cache disabled: the startup self-check "
                "raised. Tokenization will be unaffected.",
                exc_info=True,
            )
            return False, False
